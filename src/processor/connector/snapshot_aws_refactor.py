...
def get_aws_data(snapshot_json, snapshot):
    """
...
def _get_function_kwargs(arn_str, function_name, existing_json):
    ...
    {
        's3': {'Bucket' : resource_id},
        'rds': {'DBInstanceIdentifier': resource_id} if function_name in ["describe_db_instances", "describe_db_snapshots"] else {},
        'ec2': {'Attribute': 'instanceType', 'InstanceId': resource_id} if function_name == "describe_instance_attribute" else {},
...
def populate_snapshot_aws(snapshot_json, fssnapshot):
    """
    Add steps as per new interface.
    """
    ...
    account_id = get_field_value(snapshot_json, 'accountId')
    sub_data = get_aws_data(snapshot_json, fssnapshot)

    if valid_snapshotids and sub_data and fssnapshot.get_snapshot_nodes(snapshot_json):
        ...
        if access_key and secret_access:
            ...
            for node in fssnapshot.get_snapshot_nodes(snapshot_json):
                ...
                if 'snapshotId' in node and validate:
                    ...
                    if awsclient:
                        data = get_node(awsclient, node, get_field_value(snapshot_json, "source"))
                        if data:
                            ...
                            # else:
                            #     snapshot_dir = make_snapshots_dir(fssnapshot.)
                            #     if snapshot_dir:
                    ...
                    for each_region in all_regions:
                        ...
                        logger.info(awsclient)
                        if awsclient:
                            all_data = get_all_nodes(awsclient, node, snapshot_json, sub_data)
                            if all_data:
